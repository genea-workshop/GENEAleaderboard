...
<main class="subcontainer">
  <div class="home-split-ver">
    <div class="home-text card">
      ...
      <h2 class="h2">Outcomes</h2> 
      <p>Once the Leaderboard is operational, you will be able to:</p> 
      <ul> 
      ...
      <figure style="text-align: center; margin-bottom: 2em;"> 
        <video width="80%" controls><source src={resolve('/smplx_render_example.mp4' as any)} type="video/mp4"> ERROR: Your browser could not load the example video. </video> 
        <figcaption><i>An example video clip rendered from the BEAT-2 dataset. The avatar is a textured <a href="https://smpl-x.is.tue.mpg.de/" rel="external nofollow noopener" target="_blank">SMPL-X mesh</a>.</i>
        </figcaption> 
      </figure> 
      <p>We think this data is the best candidate for an initial benchmark dataset for several reasons:</p> 
      <ol> 
        <li>It’s the largest public mocap dataset of gesturing (with 60 hours of data in total).</li> 
        <li>BEAT, its predecessor, is one of the most commonly used gesture-generation datasets in recent years.</li> 
        <li>It has a high variety of speakers and emotions, and it includes semantic gesture annotations.</li> 
      ...
      <h2 class="h2">How to participate</h2> 
      <ol style="display: grid; gap: 10px"> 
        <li> <b>Pre-screening</b>: Send an e-mail to our <a href="mailto:%20genea-leaderboard@googlegroups.com">contact address</a>, including the following information: 
          <ul> 
          ...
          </ul> We will get back to you within a few days, confirming whether we can commit to evaluating your future submission.
        </li> 
        <li> <b>Prepare your model</b>: Train your model on the official training split of the BEAT-2 dataset and/or any other publicly available mocap data, except for the BEAT-2 test set. Given an arbitrarily long speech recording, your model must be able to generate an equally long motion sequence. Speaker ID, emotion labels, and the SMPL-X body shape vector will always be available as inputs, but you are not required to use them.</li> 
        <li> <b>Generate your synthetic motion</b>: For each speech recording in the BEAT-2 English test set, generate corresponding synthetic movements in the SMPL-X format. If your model is probabilistic (i.e., nondeterministic), please generate 5 samples for each input file, so that we can measure diversity.</li> 
        <li> <b>Submit your motion data and write a technical report:</b> Send your motion data to us, either through e-mail or on the submission page we will prepare for you. In order for your submission to be included on the leaderboard, you have to commit to writing a technical report about your submission, including but not limited to details about data, model architecture, training methods and generation process.</li> 
      </ol> 
      <h2 class="h2">What happens after your submission</h2> 
      <ol style="display: grid; gap: 10px"> 
      ...
      <h3 class="h3" >Motion quality</h3> 
      <p>The first evaluation task will measure motion quality, in other words, to what degree do the evaluators perceive the overall movements to be natural-looking gesturing, without considering the speech. For this evaluation, the stimuli will be <b>silent videos</b>, and we will perform pairwise comparisons of motions from different sources (e.g., gesture-generation systems, baselines, or mocap data).</p> 
      <p>The statistical analysis will use an Elo-style ranking system, in particular the Bradley-Terry model, similar to the methodology of <a href="https://lmarena.ai/" rel="external nofollow noopener" target="_blank">Chatbot Arena</a>. You can read more about Elo scores and the Bradley-Terry model in <a href="https://lmsys.org/blog/2023-12-07-leaderboard/#transition-from-online-elo-rating-system-to-bradley-terry-model" rel="external nofollow noopener" target="_blank">this blog post</a>; the key point is that 1) Elo-like systems naturally work well in a leaderboard setting where scores are continuously updated and the comparisons are not necessarily exhaustive; 2) the difference between the Elo scores of two systems directly expresses the probability that users prefer the output of one system over the output of the other.</p> 
      <p>We believe that this approach will prove to be a highly scalable and efficient method, with interpretable results, that allows us to conduct sustainable recurring evaluations for each future submission separately.</p> 
      <figure style="text-align: center; margin-bottom: 2em;"> 
        <img src={resolve('/hemvip_gui.png' as any)} style="border: 1px solid black;" width="100%">
        <figcaption><i>A preview of the evaluation interface used in our studies.</i></figcaption> 
      </figure> 
      <h3 class="h3">Motion specificity to speech</h3> 
      <p>The second evaluation task will measure whether the outputs of the gesture-generation system are <i>somehow related</i> to the speech input. As discussed in the <a href="https://dl.acm.org/doi/10.1145/3656374" rel="external nofollow noopener" target="_blank">GENEA challenge 2022 paper</a>, a naive evaluation of this question – e.g., directly asking evaluators to choose which of two systems generated movements that are more appropriate for the speech – has significant risk of confounding with other factors such as motion quality.</p> 
      <p>Therefore, we will use a mismatching procedure based on the <a href="https://arxiv.org/abs/2308.12646" rel="external nofollow noopener" target="_blank">GENEA Challenges</a>. In a nutshell, our approach will be to show two clips from the same system, one with correctly paired speech and motion, and the other with independent, intentionally misaligned motion and speech signals. Evaluators then will be tasked with indicating which of the two videos has better connection between speech and motion.</p> 
      <p>This is also a pairwise comparison, but unlike the motion quality assessment, it can be performed for each system independently, therefore it avoids the confounding factor of motion quality.</p> 
      <h3 class="h3">Future evaluations</h3> 
      <p>After the leaderboard becomes established, we will include new evaluation tasks based on what datasets become available, and what challenges become more important in the field. Some possibilities, already compatible with the BEAT2 dataset, are to evaluate facial expressions or emotion expressivity. For future datasets, it might become possible to test motion specificity to the meaning of the speech, and other types of grounding information. (See our <a href="/leaderboard/position_paper">position paper for more ideas.)</a></p> 
      <h1 class="h1" style="color: #3979c0">Tooling</h1> 
      <h2 class="h2">Standardised visualisation</h2> 
      <p>Visualisation is one of the most important design choices for perceptual user studies that evaluate motion synthesis. Currently, almost every gesture-generation paper uses a different character model and 3D scene configuration due to difficulties of using animation software, as well as the lack of shared 3D assets. Because character appearence and other environmental factors can have a subtle but important effect on the evaluation, this means that human evaluation results are largely incomparable to each other.</p> 
      <p>We will use a standardised visualisation setup, containining a textured SMPl-X mesh as a human character model, and a minimal 3D scene with lighting. There will be an option to hide the face in the videos, since our first evaluations will only be based on hand- and body motion. <strong>The videos shown above on this page are previews of our visualisation setup.</strong></p> 
      <p>We are currently working on an open-source, automated pipeline for rendering videos for our user studies, based on our previous <a href="https://github.com/TeoNikolov/genea_visualizer/" rel="external nofollow noopener" target="_blank">GENEA Blender visualiser</a>. The updated pipeline will be shared with the community after we release the leaderboard.</p> 
      <h2 class="h2">User-study automation</h2> 
      <p>To standardise the human evaluation process, we are rewriting the <a href="https://github.com/genea-workshop/hemvip" rel="external nofollow noopener" target="_blank">HEMVIP codebase</a>, which was originally developed for the GENEA challenges, with an emphasis on stability and ease of use. This software will also be open-sourced – our vision is to enable independent replication of our evaluations, and to lower the barriers for crowd-sourced evaluations.</p> 
      <h2 class="h2">Objective evaluation</h2> 
      <p>The leaderboard will also feature many commonly used objective metrics (e.g., FGD and beat consistency), and we are planning to develop new automated evaluation methods based on the collected human preference data. Each of these will be open-sourced with the release of the leaderboard.</p> 
      <h1 class="h1" style="color: #3979c0">Frequently Asked Questions</h1> 
      <details> <summary>Why do we need a leaderboard?</summary> 
        <ul> 
